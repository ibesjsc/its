...
#trainX = numpy.reshape(trainX, (trainX.shape[0], k, number_time_step)) # *chỗ này
#testX = numpy.reshape(testX, (testX.shape[0], k, number_time_step)) # *chỗ này


# create and fit the LSTM network
model = Sequential()
model.add(LSTM(4, input_shape=(number_time_step, k))) # *chỗ này # input with number_time_steps neurons (in there, each of neuron is vector (frame,Seq)) and 1st hidden layer with 9 neurons
#model.add(LSTM(9, input_shape=(k,number_time_step)))
#model.add(Dense(10))
model.add(Dense(1))
